...
function checkSys() {
	...
	_exists "apt" && apt install curl dnsutils efibootmgr file jq net-tools openssl subnetcalc tuned wget xz-utils -y
	...
	[[ "$CurrentOS" == "AlpineLinux" ]] && {
		...
		_exists "apk" && apk update
		_exists "apk" && apk add bash bind-tools coreutils cpio curl efibootmgr file gawk grep gzip ipcalc jq lsblk net-tools openssl sed shadow tzdata wget xz
